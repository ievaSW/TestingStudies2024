...
//HOMEWORK
    ...
    test.describe(`Testing marathon`, () => {
        test('Should be able to calculate birth date after and before year 2000', async ({page})=>{
        await page.goto('https://testingmarathon.com/testing/Y2K_Bank_Account_Age_Calculator/')
        const dataArray = [
            {date: '1999-12-31', name:'Ahsoka', lastname:'Thano', result:'Ahsoka Thano, Your age: 24'},
            {date: '2000-01-01', name:'Ahsoka', lastname:'Thano', result:'Ahsoka Thano, Your age (with Y2K bug): 124 years'},
        ]
    ...
    test('Date is before 2000', async ({page})=>{
        ...
        await page.locator('//button').click();
        await expect(page.locator('#result')).toContainText('Uoga Uogiene, Your age: 24 years')
    ...
    test('Date is after 2000', async ({page})=>{
        ...
        await page.locator('//button').click();
        await expect(page.locator('#result')).toContainText('Uoga Uogiene, Your age (with Y2K bug): 124 years')
